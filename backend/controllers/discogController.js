...
const searchAlbums = asyncHandler(async (req, res) => {
  ...
  const albums = data.results.map((album) => {
    ...
    if (!acc.find((album) => album.title.toString().toLowerCase() === cur.title.toString().toLowerCase() && album.artist.toString().toLowerCase() === cur.artist.toString().toLowerCase())) {
      acc.push(cur);
    }
    return acc;
  }, []);
